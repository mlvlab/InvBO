...
class Optimize(object):
    ...
    def __init__(
        ...
        if wandb_project_name: # if project name specified
            ...
            self.wandb_project_name = f"{self.task_id}"
    ...
    def log_data_to_wandb_on_each_loop(self):
        if self.track_with_wandb:
            dict_log = {
                ...
                "n_oracle_calls":self.invbo_state.objective.num_calls,
    ...
    def run_invbo(self): 
        ...
        while self.invbo_state.objective.num_calls < self.max_n_oracle_calls:
            ...
            if (self.invbo_state.progress_fails_since_last_e2e >= self.e2e_freq) and self.update_e2e:
                self.invbo_state.update_models_e2e(self.track_with_wandb, self.tracker) # Surrogate & VAE update
